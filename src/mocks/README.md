...
- [`nightly_planner.py`](nightly_planner.py) is the code under test. It's complete: you only write tests.
  `NightlyPlanner.review(result)` notifies the planner on call when the nightly solve found no plan. Its
...
- [`test_nightly_planner.py`](../../tests/mocks/test_nightly_planner.py) holds:
  - the book's two tests, finished. The first uses `RecordingNotifier`, a mock written by hand: a class whose
    `notify` appends the message to a list. The second uses `unittest.mock`, which builds such an object for
...
1. Write the body with the Arrange / Act / Assert layout of the finished tests, using `Mock(spec=Notifier)`.
...
4. In `review`, call `self._notifier.notify(...)` twice for `INFEASIBLE`. Run the tests: the "called once"
...
5. In `review`, remove the `TIME_LIMIT_NO_SOLUTION` branch. Run the tests: your time-limit test fails. Without
   a mock, nothing in the program would show that a notification was missing. **Undo the change.**
...
|---|---|
| `notifier = mock(Notifier)` | `notifier = Mock(spec=Notifier)` |
| `expect notifier.notify called once with "…"` | `notifier.notify.assert_called_once_with("…")` |
| `expect notifier.notify never called` | `notifier.notify.assert_not_called()` |

Two things catch people out:

- **Always pass `spec=`.** A bare `Mock()` accepts *any* call, including typos: `notifier.notfiy("…")` is recorded
  without complaint, and a test can pass while checking nothing. `spec=Notifier` makes the mock reject any call
  a `Notifier` doesn't have.
- **Assertions start with `assert_`.** `notifier.notify.assert_called_once_with(...)` checks. On old Python
  versions, a misspelled `notifier.notify.called_once_with(...)` was just another recorded call and checked
